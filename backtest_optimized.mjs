...
/**
 * Detectar divergências (simplificado)
 */
function detectDivergence(rsiArray, priceArray) {
  if (rsiArray.length < 5 || priceArray.length < 5) return null;
  
  const recent = Math.min(rsiArray.length, priceArray.length);
  const recentRSI = rsiArray.slice(-recent);
  const recentPrice = priceArray.slice(-recent);
  
  // Bullish: preço cai mas RSI sobe
  if (recentPrice[recentPrice.length - 1] < recentPrice[0] && recentRSI[recentRSI.length - 1] > recentRSI[0]) {
    return "BULLISH";
  }
  
  // Bearish: preço sobe mas RSI cai
  if (recentPrice[recentPrice.length - 1] > recentPrice[0] && recentRSI[recentRSI.length - 1] < recentRSI[0]) {
    return "BEARISH";
  }
...
/**
 * Gerar sinal com estratégia otimizada
 */
function generateSignal(candles4h, currentPrice) {
  ...
  // Divergências
  const rsiDivergence = detectDivergence(rsiArray4h, closes4h);
  const macdDivergence = detectDivergence(macdArray4h, closes4h);
  ...
  // BUY: RSI 30-50 + Divergência Bullish OU Crossover UP + EMA21 > EMA50
  if (rsi4h > 30 && rsi4h < 50 && ema21 > ema50) {
    if (rsiDivergence === "BULLISH" || crossedUp || macdDivergence === "BULLISH") {
      signal = "BUY";
      confidence = 60;
      if (rsiDivergence === "BULLISH") confidence += 10;
      if (crossedUp) confidence += 10;
      if (macdDivergence === "BULLISH") confidence += 5;
      if (adx > 20) confidence += 5;
    }
  }
  
  // SELL: RSI 50-70 + Divergência Bearish OU Crossover DOWN + EMA21 < EMA50
  if (rsi4h > 50 && rsi4h < 70 && ema21 < ema50) {
    if (rsiDivergence === "BEARISH" || crossedDown || macdDivergence === "BEARISH") {
      signal = "SELL";
      confidence = 60;
      if (rsiDivergence === "BEARISH") confidence += 10;
      if (crossedDown) confidence += 10;
      if (macdDivergence === "BEARISH") confidence += 5;
      if (adx > 20) confidence += 5;
    }
  }
...
async function runOptimizedBacktest() {
  console.log('🚀 Backtest de 1 Ano - Estratégia Otimizada\n');
  console.log('Filtros: Entradas 4H + Divergências RSI/MACD + RSI Relaxado (30-70)\n');
...
✅ ${symbol}:
  ...
  // Resumo
  console.log('\n' + '='.repeat(120));
  console.log('📈 RESUMO - BACKTEST 1 ANO (Estratégia Otimizada)');
